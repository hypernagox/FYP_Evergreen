        ...
        void Export()
        {
            ...
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    ...
                    if (x < w - 1 && y < h - 1)
                    {
                        Vector3 right = tVertices[y * w + (x + 1)] - tVertices[y * w + x];
                        Vector3 down = tVertices[(y + 1) * w + x] - tVertices[y * w + x];
                        tNormals[y * w + x] = Vector3.Cross(down, right).normalized;
                    }
                    else
                    {
                        tNormals[y * w + x] = Vector3.up; // Use an upward normal for edge cases
                    }
                }
            }
            ...
            try
            {
                ...
                System.Threading.Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo("en-US");
                counter = tCount = 0;
                totalCount = (tVertices.Length * 3 + (saveFormat == SaveFormat.Triangles ? tPolys.Length / 3 : tPolys.Length / 4)) / progressUpdateInterval;
                for (int i = 0; i < tVertices.Length; i++)
                {
                    UpdateProgress();
                    StringBuilder sb = new StringBuilder("v ", 20);
                ...
                {
                    UpdateProgress();
                    StringBuilder sb = new StringBuilder("vn ", 20);
                ...
                if (saveFormat == SaveFormat.Triangles)
                {
                    // Write triangles with normals
                    for (int i = 0; i < tPolys.Length; i += 3)
                    {
                        UpdateProgress();
                        StringBuilder sb = new StringBuilder("f ", 43);
                        sb.Append(tPolys[i] + 1).Append("/").Append(tPolys[i] + 1).Append("/").Append(tPolys[i] + 1).Append(" ").
                           Append(tPolys[i + 1] + 1).Append("/").Append(tPolys[i + 1] + 1).Append("/").Append(tPolys[i + 1] + 1).Append(" ").
                           Append(tPolys[i + 2] + 1).Append("/").Append(tPolys[i + 2] + 1).Append("/").Append(tPolys[i + 2] + 1);
                        sw.WriteLine(sb);
                    }
                }
                else
                {
                    // Write quads with normals
                    for (int i = 0; i < tPolys.Length; i += 4)
                    {
                        UpdateProgress();
                        StringBuilder sb = new StringBuilder("f ", 57);
                        sb.Append(tPolys[i] + 1).Append("/").Append(tPolys[i] + 1).Append("/").Append(tPolys[i] + 1).Append(" ").
                           ...
                           Append(tPolys[i + 3] + 1).Append("/").Append(tPolys[i + 3] + 1).Append("/").Append(tPolys[i + 3] + 1);
                        sw.WriteLine(sb);
                    }
                }
            }
