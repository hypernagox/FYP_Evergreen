...
public:
	template<typename T>
	void BindParam(const int32_t idx, T&& value)noexcept
	{
		m_dbConnection->BindParam(idx + 1, &value, &m_paramIndex[idx]);
		m_paramFlag |= (1LL << idx);
	}

	void BindParam(const int32_t idx, std::string& value)noexcept
	{
		m_dbConnection->BindParam(idx + 1, value.data(), &m_paramIndex[idx]);
		m_paramFlag |= (1LL << idx);
	}

	void BindParam(const int32_t idx, std::wstring& value)noexcept
	{
		m_dbConnection->BindParam(idx + 1, value.data(), &m_paramIndex[idx]);
		m_paramFlag |= (1LL << idx);
	}

	void BindParam(const int32_t idx, const WCHAR* const value)noexcept
	{
		m_dbConnection->BindParam(idx + 1, value, &m_paramIndex[idx]);
		m_paramFlag |= (1LL << idx);
	}
	...
	void BindParam(const int32_t idx, T(&value)[N])noexcept
	{
	...
	template<typename T>
	void BindParam(const int32_t idx, T* const value, const int32_t N)noexcept
	{
		m_dbConnection->BindParam(idx + 1, (std::byte* const)value, size32(T) * N, &m_paramIndex[idx]);
		m_paramFlag |= (1LL << idx);
	}
	...
	template<c_int32 N>
	void BindCol(const int32_t idx, WCHAR(&value)[N])noexcept
	{
		m_dbConnection->BindCol(idx + 1, value, N - 1, &m_columnIndex[idx]);
		m_columnFlag |= (1LL << idx);
	}

	void BindCol(const int32_t idx, WCHAR* const value, const int32_t len)noexcept
	{
		m_dbConnection->BindCol(idx + 1, value, len - 1, &m_columnIndex[idx]);
		m_columnFlag |= (1LL << idx);
	}

	void BindCol(const int32_t idx, CHAR* const value, const int32_t len)noexcept
	{
		m_dbConnection->BindCol(idx + 1, value, len - 1, &m_columnIndex[idx]);
		m_columnFlag |= (1LL << idx);
	}

	void BindCol(const int32_t idx, std::string& value)noexcept
	{
		m_dbConnection->BindCol(idx + 1, value.data(), (c_int32)value.size(), &m_columnIndex[idx]);
		m_columnFlag |= (1LL << idx);
	}

	void BindCol(const int32_t idx, std::wstring& value)noexcept
	{
		m_dbConnection->BindCol(idx + 1, value.data(), (c_int32)value.size(), &m_columnIndex[idx]);
		m_columnFlag |= (1LL << idx);
	}
	...
	template<typename T, c_int32 N>
	void BindCol(const int32_t idx, T(&value)[N])noexcept
	{
...
private:
	...
	uint64_t m_paramFlag = {};
	uint64_t m_columnFlag = {};
};
