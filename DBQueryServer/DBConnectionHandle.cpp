...
const bool DBConnectionHandle::Connect(const SQLHENV henv, const std::wstring_view connectionString)noexcept
{
	...
	{
		//std::wcout << L"[ODBC 연결 실패]\n";
		//std::wcout << L"  SQLSTATE: " << sqlState << L"\n";
		//std::wcout << L"  오류 코드: " << nativeError << L"\n";
		//std::wcout << L"  메시지: " << message << L"\n";
	}
...
const bool DBConnectionHandle::BindParam(c_int32 paramIndex, int8_t* const value, SQLLEN* const index)const noexcept
{
	return BindParam(paramIndex, SQL_C_TINYINT, SQL_TINYINT, size32(int8_t), value, index);
}

const bool DBConnectionHandle::BindParam(c_int32 paramIndex, int16_t* const value, SQLLEN* const index)const noexcept
{
	return BindParam(paramIndex, SQL_C_SHORT, SQL_SMALLINT, size32(int16_t), value, index);
}

const bool DBConnectionHandle::BindParam(c_int32 paramIndex, int32_t* const value, SQLLEN* const index)const noexcept
{
	return BindParam(paramIndex, SQL_C_LONG, SQL_INTEGER, size32(int32_t), value, index);
}

const bool DBConnectionHandle::BindParam(c_int32 paramIndex, int64_t* const value, SQLLEN* const index)const noexcept
{
...
const bool DBConnectionHandle::BindParam(c_int32 paramIndex, std::byte* const bin, int32_t size, SQLLEN* const index)const noexcept
{
	if (bin == nullptr)
	{
		*index = SQL_NULL_DATA;
...
const bool DBConnectionHandle::BindCol(c_int32 columnIndex, int8_t* const value, SQLLEN* const index)const noexcept
{
	return BindCol(columnIndex, SQL_C_TINYINT, size32(int8), value, index);
}

const bool DBConnectionHandle::BindCol(c_int32 columnIndex, int16_t* const value, SQLLEN* const index)const noexcept
{
	return BindCol(columnIndex, SQL_C_SHORT, size32(int16_t), value, index);
}

const bool DBConnectionHandle::BindCol(c_int32 columnIndex, int32_t* const value, SQLLEN* const index)const noexcept
{
	return BindCol(columnIndex, SQL_C_LONG, size32(int32_t), value, index);
}

const bool DBConnectionHandle::BindCol(c_int32 columnIndex, int64_t* const value, SQLLEN* const index)const noexcept
{
