...
NodeStatus RangeCheckNode::Tick(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer, const NagiocpX::S_ptr<NagiocpX::ContentsEntity>& awaker) noexcept
{
    ...
    if (m_bReEvaluate)
    {
       ...
       // const auto session_list = pOwnerEntity->GetCurSector()->GetSessionCopyListIncRef();
        ...
        if (target)
        {
            bt_root_timer->SetCurAwakerID(target->GetObjectID());
   ...
   // std::cout << "공격!" << std::endl;
    ...
    if (0.f >= m_accTime)
    {
       ...
       // awaker->GetCurCluster()->Broadcast(Create_s2c_MONSTER_ATTACK(owner_comp_sys->GetOwnerEntity()->GetObjectID(), awaker->GetObjectID(), 1));
        awaker->GetComp<HP>()->PostDoDmg(2, owner_comp_sys->GetOwnerEntity()->SharedFromThis());
