...
public:
    ...
    std::wstring m_name;
    std::wstring m_pw;
    std::wstring m_class_type = L"CharacterNonSelect";
    ...
    NagiocpX::DBBindRAII<1, 2> dbSelectUID{
     L"SELECT UID, Type FROM Users WHERE Name = ?"
    };
...
class DB_NewPlayerInsert :
    public NagiocpX::DBEvent
{
public:
    DB_NewPlayerInsert(S_ptr<NagiocpX::Session> pPlayerSession)
        : NagiocpX::DBEvent{ std::move(pPlayerSession) }
    {
    }
private:
...
public:
    ...
    std::wstring m_name;
    std::wstring m_pw;
    std::wstring m_class_type = L"Warrior";
    RegisterResult m_result = RegisterResult::NONE;
    int64_t m_db_uid = 0;
    XVector<int> m_item_ids;
    XVector<int> m_item_counts;
    int m_weapon_id = -1;
