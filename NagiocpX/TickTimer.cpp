...
namespace NagiocpX
{
	...
	void TickTimer::Tick(S_ptr<ContentsEntity>* const owner_entity) noexcept
	{
		...
		NagiocpX::ClusterInfoHelper::FillterSessionEntities(obj_insight, GetOwnerEntity());
		...
		const TIMER_STATE eCurState = obj_insight.empty() ? TIMER_STATE::IDLE : TimerUpdate(NagiocpX::Service::GetMainService()->GetSession(m_curAwakerID));
...
const NagiocpX::TIMER_STATE TickTimerBT::TimerUpdate(const S_ptr<NagiocpX::ContentsEntity> awaker) noexcept
{
	...
	if (NodeStatus::FAILURE == cur_status && NodeStatus::FAILURE == m_prevStatus)
	{
		...
		return NagiocpX::TIMER_STATE::IDLE;
	}

	m_prevStatus = cur_status;
	return NagiocpX::TIMER_STATE::RUN;
}
...
const NagiocpX::TIMER_STATE TickTimerFSM::TimerUpdate(const S_ptr<NagiocpX::ContentsEntity> awaker) noexcept
{
	DO_BENCH_GLOBAL_THIS_FUNC;
	const auto pOwnerEntity = GetOwnerEntity();
	const auto owner_comp_sys = static_cast<const ComponentSystemNPC* const>(pOwnerEntity->GetComponentSystem());
