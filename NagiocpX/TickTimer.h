...
namespace NagiocpX
{
	...
	protected:
		virtual const NagiocpX::TIMER_STATE TimerUpdate(const S_ptr<ContentsEntity> awaker)noexcept = 0;
...
class TickTimerBT
	:public NagiocpX::TickTimer
{
public:
	TickTimerBT(NagiocpX::ContentsEntity* const pOwner_, CompositeNode* const rootNode, const uint16_t awakeDist_ = UINT16_MAX)noexcept
...
private:
	virtual const  State::EntityState Update(const float dt, const ComponentSystemNPC* const comp_sys,const S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept = 0;
	virtual void Enter(const float dt, const ComponentSystemNPC* const comp_sys, const S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept = 0;
	virtual void Exit(const float dt, const ComponentSystemNPC* const comp_sys, const S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept = 0;
...
{
public:
	TickTimerFSM(NagiocpX::ContentsEntity* const pOwner_, const uint16_t awakeDist_ = UINT16_MAX)noexcept
		: TickTimer{ pOwner_ ,awakeDist_ }
	{
	}
	virtual ~TickTimerFSM()noexcept { for (const auto state : m_states)NagiocpX::xdelete<State>(state.second); }
