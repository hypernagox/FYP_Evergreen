...
namespace NagiocpX
{
	...
	protected:
		...
		virtual void OnDisconnected(const NagiocpX::Cluster* const curCluster_)noexcept = 0;
	private:
		// ----- SEND ----
...
static inline void operator <<(SESSION&& s, NagiocpX::S_ptr<NagiocpX::SendBuffer>& b)noexcept
{
	s->SendAsync(b);
}
template <typename SESSION>
