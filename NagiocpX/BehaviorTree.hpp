...
public:
    ...
    virtual NodeStatus Tick(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer, const NagiocpX::S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept = 0;
...
{
public:
    NodeStatus Tick(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer, const NagiocpX::S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept override = 0;
private:
...
public:
    ...
    ~DecoratorNode()noexcept { NagiocpX::xdelete<BehaviorNode>(m_childNode); }
    virtual NodeStatus Tick(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer, const NagiocpX::S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept override = 0;
    virtual void Reset(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer)noexcept override = 0;
protected:
...
public:
    ...
    virtual NodeStatus Tick(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer, const NagiocpX::S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept override { return ReapeatTick(m_childNode->Tick(owner_comp_sys, bt_root_timer, awaker)); }
    virtual void Reset(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer)noexcept override = 0;
private:
...
public:
    virtual ~CompositeNode()noexcept {
        ...
        while (e != b) { NagiocpX::xdelete<BehaviorNode>(*b++); }
    }

    template <typename T, typename... Args>
    T* const AddChild(Args&&... args)noexcept {
        return static_cast<T* const>(m_vecChildren.emplace_back(NagiocpX::xnew<T>(std::forward<Args>(args)...)));
    }
...
protected:
    NagiocpX::XVector<BehaviorNode*> m_vecChildren;
...
{
public:
    NodeStatus Tick(const ComponentSystemNPC* const owner_comp_sys, TickTimerBT* const bt_root_timer, const NagiocpX::S_ptr<NagiocpX::ContentsEntity>& awaker)noexcept override final
    {
