...
ComPtr<ID3D12PipelineState> CylinderParticleEmitter::m_pipelineState = nullptr;
std::default_random_engine CylinderParticleEmitter::m_randomEngine{};
...
static ComPtr<ID3D12PipelineState> InitPipelineState(ComPtr<ID3D12Device> device)
{
	...
	auto vsByteCode = DX::ReadData(L"compiled_shaders\\vs_cylinder_particle_emitter.cso");
...
void CylinderParticleEmitter::OnInitialize()
{
...
void CylinderParticleEmitter::Update(const udsdx::Time& time, udsdx::Scene& scene)
{
	if (m_isPlaying)
	{
		if (time.totalTime - m_emitterParameter.EmitTime >= m_emitterParameter.LifeTimeMax && !m_emitLoop)
		{
			m_isPlaying = false;
			if (m_autoDestroy)
			{
...
void CylinderParticleEmitter::PostUpdate(const udsdx::Time& time, udsdx::Scene& scene)
{
...
void CylinderParticleEmitter::Render(udsdx::RenderParam& param, int parameter)
{
	unsigned int flags =
...
void CylinderParticleEmitter::UpdateTransformCache()
{
	m_transformCache = GetSceneObject()->GetTransform()->GetWorldSRTMatrix(false);
}

void CylinderParticleEmitter::Play()
{
	m_isPlaying = true;
	m_emitterParameter.EmitTime = ET;
}
